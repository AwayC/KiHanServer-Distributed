...
`game_server` 负责处理游戏内的核心战斗逻辑和帧同步驱动。它同时与 `lobby_server` (通过内网 gRPC 直接通信) 和 客户端 (通过 Gateway 的 gRPC 双向流) 进行交互。
...
这些接口是服务端之间的内部调用，通常使用标准 gRPC (Unary RPC)。为了结构清晰，我们将这部分定义在独立的 gRPC Service 中（如 `GameControlService`）。

### 1.1 `CreateRoom` (Lobby -> Game)
*   **描述**: 大厅服匹配成功后，请求战斗服创建一个独立的房间。
*   **请求 Payload (`CreateRoomReq`)**: 
    包含 1P 和 2P 的玩家信息 (`uid`, `nickname`, `character_id`)。
...
*   **请求 Payload (`GameResultNtf`)**: 
    包含 `room_id`, `1p_uid`, `2p_uid`, 以及 `winner_uid` (0代表平局)。

---

## 2. 客户端接入协议 (Client <-> Gateway <-> Game)
客户端通过 Gateway 以 `CmdID >= 2000` 的包与 `game_server` 通信。底层走的是 gRPC 的 **双向流 (Bidirectional Streaming)**。

### 通用数据结构
```protobuf
message RoomSnapshot {
    int32 room_id = 1;
    string player_list_json = 2; // 包含 1P 和 2P 的完整信息，如: [{"uid":"...", "nickname":"...", "character_id":1}, ...]
}

message InputFrame {
    uint32 frame_id = 1;
    uint32 joystick = 2; // 摇杆输入 (压缩)
    uint32 buttons = 3;  // 按钮输入 (按位压缩)
}

message RoomFrame {
    uint32 frame_id = 1;
    uint32 player_count = 2;
    map<int32, InputFrame> inputs = 3; // key 为 game_id (1 或 2)
}
```
...
*   **请求: PlayerFrameInput**
    *   描述：客户端高频上报自己的操作帧。
    *   Payload：`message PlayerFrameInput { InputFrame input = 1; }`
*   **响应**: 无直接响应，整合后通过 2005 下发。
...
*   **请求: GameOverReq**
    ...
    *   Payload：`message GameOverReq { int32 room_id = 1; string winner_uid = 2; int32 p1_hp = 3; int32 p2_hp = 4; }`
*   **响应: GameOverRsp**
    *   Payload：`message GameOverRsp { int32 err_code = 1; }`

### CmdID: 2007 - 服务器裁定游戏结束通知 (Server 推送)
*   **通知: GameOverNtf**
    *   描述：服务器收到结算请求，或某方异常掉线断开，强制结束游戏并广播结果。
    *   Payload：`message GameOverNtf { int32 room_id = 1; string winner_uid = 2; }`
...
2.  **帧同步机制**:
    ...
    *   **广播**: Tick 触发时，封装 `RoomFrame` 并广播给房间内所有有效玩家。